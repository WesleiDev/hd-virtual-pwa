...
export default function(ref){
    ...
    for(let index in foldersPath){
        ...
        breadcrumbs += ` / <a href="" data-type="folder-open" data-fid="${foldersPath[index].id}" 
        data-title="${foldersPath[index].fid}">${foldersPath[index].title}</a>`;
    }

    let database = app.database();
    let filesRef = database.ref(firebase_ref);
    filesRef.on("value", (snapshot) =>{getData(snapshot)})
